...
name: qa-analyzer
description: >
  QA Audit agent that cross-references expected results from the test plan with
  raw observations from the execution engine. Produces a machine-readable audit
  log with PASS/FAIL/BLOCKED verdicts and defect context.
  Summon via @qa-analyzer or /qa-analyzer.
---
...
> **Path resolution:** Read `.agents/core-workflow-config.md` in the project root to find the **workflow directory**. All paths below are relative to the project root.

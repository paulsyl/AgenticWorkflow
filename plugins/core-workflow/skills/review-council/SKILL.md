...
name: review-council
description: >
  Multi-persona code review council that validates the Architect's plan against
  the PRD. 4 core personas always run (Security & Resilience, Data Integrity,
  Pragmatism & Scope, Testability). Optional personas (Performance, UI/UX,
  Deployment) are invoked when the change touches those areas.

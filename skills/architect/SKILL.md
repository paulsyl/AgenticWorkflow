...
name: architect
description: >
  Translate a rigid PRD contract into technical blueprints. Produces vertical-sliced Phase
  files with plain-English execution steps. Each phase is a demoable feature slice through
  all layers (schema, API, UI, tests), not a horizontal layer. Reads only the PRD — no
  separate scope, context, or architecture documents. Summon via @architect or /architect.
---
